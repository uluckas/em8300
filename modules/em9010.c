...
static
int loc_2BE50(struct em8300_s *em) {
    em9010_write(em,0xb, 0xc8);
    sub_2AC2D(em);
...
int em9010_calibrate_yoffset(struct em8300_s *em)
{
	...
	em8300_dicom_fill_dispbuffers(em,0,0,em->dbuf_info.xsize,4,
				      0xffffffff,0x80808080 );
    else
	em8300_dicom_fill_dispbuffers(em,0,0,em->dbuf_info.xsize,2,
				      0xffffffff,0x80808080 );

    sub_4288c(em, 0, 0, em->dbuf_info.xsize, em->dbuf_info.ysize,
	...
	if(! (em9010_read(em,0) & 4)) {
	    sub_4288c(em, 0, i, em->dbuf_info.xsize, em->dbuf_info.ysize,
		      em->overlay_a[EM9010_ATTRIBUTE_XOFFSET],
...
int em9010_calibrate_xoffset(struct em8300_s *em)
{
    ...
    em8300_dicom_fill_dispbuffers(em,0,0,em->dbuf_info.xsize,4,
				      0,0x80808080 );
    em8300_dicom_fill_dispbuffers(em,2,0,2,em->dbuf_info.ysize, 
				  0xffffffff,0x80808080 ); 
	...
	if(!sub_2AC2D(em))
	    return 0;
	if(! (em9010_read(em,0) & 4)) {
	    sub_4288c(em, i, 0, em->dbuf_info.xsize, em->dbuf_info.ysize,
		      em->overlay_a[EM9010_ATTRIBUTE_XOFFSET],
		      em->overlay_a[EM9010_ATTRIBUTE_YOFFSET],
		      em->overlay_a[EM9010_ATTRIBUTE_XCORR], em->overlay_double_y);
	    mdelay(10);
...
static
int color_cal(struct em8300_s *em,int ul, int a, int b, int c,int d, int *res)
{
    int i;
...
int em9010_calibrate_xcorrection(struct em8300_s *em)
{
    ...
    em8300_dicom_fill_dispbuffers(em,2,0,2,em->dbuf_info.ysize,
				      0,0x80808080 );
    em8300_dicom_fill_dispbuffers(em,356,0,2,em->dbuf_info.ysize, 
				  0xffffffff,0x80808080 ); 
...
int loc_2A66E(struct em8300_s *em) {
    ...
    if((em->overlay_xcorr_default < 0x352) ||
       (em->overlay_xcorr_default > 0xdac)) 
...
int em8300_ioctl_overlay_calibrate(struct em8300_s *em,
				   em8300_overlay_calibrate_t *c)
{
    em9010_write(em, 0xc, 0xe);
	...
	if(em9010_calibrate_xcorrection(em)) {
	    ...
	    em8300_dicom_fill_dispbuffers(em,0x164,0,2,em->dbuf_info.ysize,
					  0x0,0x80808080 ); 
